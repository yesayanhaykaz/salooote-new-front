import AboutPageClient from "@/components/AboutPageClient";

export function generateStaticParams() {
  return [{ lang: "en" }, { lang: "hy" }, { lang: "ru" }];
}

export async function generateMetadata({ params }) {
  const { lang } = await params;
  return {
    title: "Our Story — Salooote.am",
    description: "Learn how Salooote was founded to make event planning in Armenia effortless.",
    alternates: {
      canonical: `https://development.salooote.am/${lang}/about`,
      languages: { en: "https://development.salooote.am/en/about", hy: "https://development.salooote.am/hy/about", ru: "https://development.salooote.am/ru/about" },
    },
    openGraph: {
      title: "Our Story — Salooote.am",
      description: "Learn how Salooote was founded to make event planning in Armenia effortless.",
      url: `https://development.salooote.am/${lang}/about`,
      siteName: "Salooote.am",
      images: [{ url: "/images/hero-dj.jpg", width: 1200, height: 630, alt: "Salooote.am" }],
      type: "website",
    },
    twitter: {
      card: "summary_large_image",
      title: "Our Story — Salooote.am",
      description: "Learn how Salooote was founded to make event planning in Armenia effortless.",
      images: ["/images/hero-dj.jpg"],
    },
  };
}

export default async function AboutPage({ params }) {
  const { lang } = await params;
  return <AboutPageClient lang={lang} />;
}
