...
const C = {
  ...
  gradOrange:   "linear-gradient(135deg,#f97316,#e11d5c)",
  gradPurple:   "linear-gradient(135deg,#7c3aed,#a855f7)",
};
...
const EVENT_TEMPLATES = {
  christening: {
    ...
    accent:C.purple, gradient:"linear-gradient(135deg,#7c3aed,#a855f7)",
    services:[
  ...
  wedding:{
    label:"Wedding", emoji:"💍",
    accent:C.pink, gradient:"linear-gradient(135deg,#e11d5c,#f97316)",
    services:[
  ...
  corporate:{
    ...
    accent:"#64748b", gradient:"linear-gradient(135deg,#475569,#1e293b)",
    services:[
  ...
  for (const a of actions) {
    switch (a.type) {
      case "set_event_type": {
        ...
        state = { ...state, event_type:a.event_type, event_type_label:tpl?.label||a.event_type, event_type_emoji:tpl?.emoji||"🎉", accent:tpl?.accent||C.pink, gradient:tpl?.gradient,
      ...
      case "remove_service": state={...state,services:(state.services||[]).filter(s=>s.service_type!==a.service_type)}; break;
...
function Orb({ size=72 }) {
  return (
    ...
    >
      {/* Glow ring */}
      <div style={{ position:"absolute", inset:-size*0.2, borderRadius:"50%", background:"radial-gradient(circle,rgba(168,85,247,0.35) 0%,transparent 70%)", pointerEvents:"none" }} />
      {/* Sphere */}
      <div style={{ width:"100%", height:"100%", borderRadius:"50%", background:"radial-gradient(circle at 35% 28%, #f0abfc 0%, #a855f7 30%, #7c3aed 58%, #1e1b4b 100%)", boxShadow:`0 0 ${size*0.4}px rgba(168,85,247,0.55), 0 ${size*0.15}px ${size*0.4}px rgba(0,0,0,0.6)`, overflow:"hidden", position:"relative" }}>
        {/* Shine */}
        <div style={{ position:"absolute", top:"12%", left:"18%", width:"38%", height:"32%", borderRadius:"50%", background:"rgba(255,255,255,0.32)", filter:"blur(4px)", transform:"rotate(-25deg)" }} />
      </div>
    </motion.div>
  );
}
...
function BotAvatar({ size=28 }) {
  return (
    <div style={{ width:size, height:size, borderRadius:"50%", flexShrink:0, background:"radial-gradient(circle at 35% 30%,#f0abfc,#7c3aed 60%,#1e1b4b)", boxShadow:"0 0 10px rgba(168,85,247,0.5)" }}>
      <div style={{ width:"100%", height:"100%", borderRadius:"50%", overflow:"hidden", position:"relative" }}>
        <div style={{ position:"absolute", top:"14%", left:"18%", width:"35%", height:"30%", borderRadius:"50%", background:"rgba(255,255,255,0.3)", filter:"blur(2px)" }} />
      </div>
    </div>
  );
}
...
function TypingIndicator() {
  return (
    <div style={{ display:"flex", alignItems:"flex-end", gap:8 }}>
      <BotAvatar />
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, backdropFilter:"blur(20px)", borderRadius:"18px 18px 18px 4px", padding:"12px 16px", display:"flex", gap:5, alignItems:"center" }}>
        {[0,0.15,0.30].map((d,i)=>(
  ...
  return (
    <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{duration:0.18,ease:[0.22,1,0.36,1]}}
      ...
      <div style={{display:"flex",alignItems:"flex-end",gap:8,maxWidth:"86%"}}>
        {isBot && <BotAvatar />}
        <div style={{
          background: isBot ? "rgba(255,255,255,0.07)" : "linear-gradient(135deg,rgba(249,115,22,0.85),rgba(225,29,92,0.85))",
          border: isBot ? `1px solid ${C.border}` : "none",
          backdropFilter: isBot ? "blur(20px)" : "none",
          color: C.text,
          borderRadius: isBot ? "18px 18px 18px 4px" : "18px 18px 4px 18px",
          padding:"11px 15px", fontSize:"0.875rem", lineHeight:1.65, letterSpacing:"-0.003em",
        }}>
...
function VendorCard({ vendor, onSelect }) {
  ...
  return (
    <motion.div onHoverStart={()=>setHov(true)} onHoverEnd={()=>setHov(false)}
      ...
      style={{
        background: hov ? "rgba(255,255,255,0.1)" : C.surface,
        border:`1px solid ${hov ? "rgba(249,115,22,0.5)" : C.border}`,
        backdropFilter:"blur(20px)",
        borderRadius:14, minWidth:156, maxWidth:176, flexShrink:0,
        overflow:"hidden", cursor:"pointer",
        boxShadow: hov ? "0 0 20px rgba(249,115,22,0.2)" : "none",
        transition:"all 0.2s",
      }}>
      {/* gradient top bar */}
      <div style={{height:2,background:hov?"linear-gradient(90deg,#f97316,#e11d5c)":"rgba(255,255,255,0.08)"}} />
      <div style={{padding:"11px 12px 12px"}}>
        <div style={{width:34,height:34,borderRadius:9,background:"rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.95rem",fontWeight:800,color:C.text,marginBottom:8,border:`1px solid ${C.border}`}}>
          {initial}
        </div>
        ...
        <motion.button whileTap={{scale:0.96}} onClick={()=>onSelect(vendor)}
          style={{
            width:"100%",background:hov?"linear-gradient(135deg,#f97316,#e11d5c)":"rgba(255,255,255,0.07)",
            border:`1px solid ${hov?"transparent":C.border}`,borderRadius:8,
            color:C.text,fontSize:"0.7rem",fontWeight:600,padding:"6px 0",cursor:"pointer",
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0"}}>
        ...
        <div style={{width:20,height:20,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
          {isSelected?(
            <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type:"spring",stiffness:460,damping:22}}
              style={{width:20,height:20,borderRadius:"50%",background:"rgba(34,197,94,0.15)",border:"1.5px solid rgba(34,197,94,0.5)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <Check size={11} color={C.green} strokeWidth={2.5}/>
            </motion.div>
          ):isSearching?(
            <motion.div animate={{rotate:360}} transition={{duration:0.9,repeat:Infinity,ease:"linear"}}
              style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${(accent||C.purple)}44`,borderTopColor:accent||C.purple}}/>
          ):(
        ...
        <div style={{flex:1,minWidth:0}}>
          <p style={{margin:0,fontSize:"0.8rem",fontWeight:600,letterSpacing:"-0.008em",color:isSelected?"rgba(134,239,172,0.9)":C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
            {service.title}
          </p>
          {isSelected&&selectedVendor&&<p style={{margin:"1px 0 0",fontSize:"0.7rem",color:"rgba(134,239,172,0.7)",fontWeight:500}}>{selectedVendor.name}</p>}
          {!service.required&&!isSelected&&<span style={{fontSize:"0.6rem",background:"rgba(255,255,255,0.06)",color:C.text3,border:`1px solid ${C.border}`,borderRadius:100,padding:"1px 6px",fontWeight:600}}>optional</span>}
        </div>
        <div style={{flexShrink:0,display:"flex",alignItems:"center"}}>
          {isSelected?(
            ...
            <span style={{fontSize:"0.66rem",background:"rgba(251,191,36,0.12)",color:"#fbbf24",fontWeight:700,borderRadius:100,padding:"2px 8px",border:"1px solid rgba(251,191,36,0.25)"}}>
              {results.length} found
            </span>
          ):service.canSearch&&!isSearching?(
            <motion.button whileHover={{scale:1.04}} whileTap={{scale:0.96}}
              onClick={()=>onSearchVendors(service.service_type,service.title)}
              style={{display:"inline-flex",alignItems:"center",gap:3,background:"rgba(255,255,255,0.06)",border:`1px solid ${C.border}`,color:C.text2,borderRadius:8,padding:"5px 10px",fontSize:"0.7rem",fontWeight:600,cursor:"pointer",transition:"all 0.15s"}}
              onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,0.1)";e.currentTarget.style.color=C.text;}}
              onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.06)";e.currentTarget.style.color=C.text2;}}>
...
function CategorySection({label,services,selectedVendors,...rowProps}){
  ...
  return (
    <div style={{background:C.surface,border:`1px solid ${C.border}`,backdropFilter:"blur(20px)",borderRadius:14,marginBottom:6,overflow:"hidden"}}>
      <button onClick={()=>setOpen(o=>!o)}
        ...
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          ...
          {doneCount>0&&(
            <span style={{fontSize:"0.62rem",fontWeight:700,borderRadius:100,padding:"2px 7px",background:allDone?"rgba(34,197,94,0.15)":"rgba(255,255,255,0.06)",color:allDone?"#86efac":C.text3,border:`1px solid ${allDone?"rgba(34,197,94,0.3)":C.border}`}}>
            ...
            <div style={{padding:"0 14px 8px",borderTop:`1px solid ${C.border}`}}>
              {services.map((s,idx)=>(
                <div key={s.service_type}>
                  {idx>0&&<div style={{height:1,background:"rgba(255,255,255,0.04)"}}/>}
...
function Chip({icon,children}){
  return (
    <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:"0.72rem",background:"rgba(255,255,255,0.15)",borderRadius:100,padding:"3px 9px",fontWeight:500,color:C.text}}>
      {icon}{children}
    </span>
  );
}
  ...
  return (
    <div style={{maxWidth:700}}>
      ...
      <div style={{background:gradient||C.grad,borderRadius:18,padding:"20px 22px",marginBottom:14,color:"#fff",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,backgroundImage:"radial-gradient(circle,rgba(255,255,255,0.08) 1px,transparent 1px)",backgroundSize:"18px 18px",pointerEvents:"none"}}/>
        <div style={{position:"absolute",top:-50,right:-50,width:170,height:170,borderRadius:"50%",background:"rgba(255,255,255,0.06)",pointerEvents:"none"}}/>
      ...
      {missing.length>0&&sel>0&&(
        <motion.div initial={{opacity:0,y:-4}} animate={{opacity:1,y:0}}
          style={{background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.25)",borderRadius:12,padding:"10px 14px",marginBottom:10,display:"flex",gap:10,alignItems:"flex-start"}}>
          <span style={{fontSize:"0.95rem",flexShrink:0,lineHeight:1.5}}>⚠️</span>
          <div>
            <p style={{margin:"0 0 2px",fontSize:"0.775rem",fontWeight:700,color:"#fbbf24"}}>Still needed:</p>
            <p style={{margin:0,fontSize:"0.72rem",color:"rgba(251,191,36,0.7)",lineHeight:1.5}}>{missing.map(s=>s.title).join(" · ")}</p>
          </div>
        </motion.div>
      )}
...
/* ═══════════════════════════════════════════════════════
   CHAT WELCOME STATE  (left panel, Touri-style)
═══════════════════════════════════════════════════════ */
const FEATURES = [
  { icon: <ListChecks size={18} color={C.orange}/>, title:"Smart Checklist", desc:"Complete checklist built instantly from your event type" },
  { icon: <Store size={18} color={C.pink}/>,        title:"Find Vendors",    desc:"Real vendors searched in your city, instantly" },
  { icon: <Heart size={18} color={C.purple}/>,      title:"Cultural Tips",   desc:"Armenian traditions & cultural guidance included" },
];

function ChatWelcome({ onPickType }) {
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",padding:"24px 20px",textAlign:"center"}}>
      <motion.div initial={{scale:0.8,opacity:0}} animate={{scale:1,opacity:1}} transition={{type:"spring",stiffness:200,damping:20,delay:0.1}} style={{marginBottom:24}}>
        <Orb size={72}/>
      </motion.div>

      <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.25}} style={{marginBottom:28,maxWidth:340}}>
        <h2 style={{margin:"0 0 12px",fontSize:"1.35rem",fontWeight:700,color:C.text,letterSpacing:"-0.03em",lineHeight:1.35}}>
          Hello! I'm your{" "}
          ...
          {" "}ready to create the{" "}
          <strong style={{fontWeight:800}}>perfect celebration.</strong>
        </h2>
        <p style={{margin:0,fontSize:"0.85rem",color:C.text2,lineHeight:1.6}}>
          Just describe your event — type, guests, city, style.<br/>I'll handle the rest.
        </p>
      </motion.div>

      {/* Feature cards */}
      <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:0.35}}
        style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,width:"100%",maxWidth:380}}>
        {FEATURES.map((f,i)=>(
          <motion.div key={i} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.38+i*0.07}}
            style={{background:C.surface,border:`1px solid ${C.border}`,backdropFilter:"blur(20px)",borderRadius:14,padding:"14px 12px",textAlign:"left"}}>
            <div style={{marginBottom:8}}>{f.icon}</div>
            <p style={{margin:"0 0 4px",fontSize:"0.75rem",fontWeight:700,color:C.text}}>{f.title}</p>
            <p style={{margin:0,fontSize:"0.67rem",color:C.text3,lineHeight:1.5}}>{f.desc}</p>
          </motion.div>
        ))}
      </motion.div>
    </div>
...
/* ═══════════════════════════════════════════════════════
   RIGHT PANEL EMPTY STATE  (event type picker)
═══════════════════════════════════════════════════════ */
const EVENT_TYPES_GRID = [
  {key:"christening",label:"Christening",emoji:"⛪",color:"#a855f7"},
  ...
  {key:"corporate",  label:"Corporate",  emoji:"🏢",color:"#64748b"},
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",padding:"32px 24px",textAlign:"center"}}>
      <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.1}} style={{marginBottom:28}}>
        ...
        <h3 style={{margin:0,fontSize:"1.2rem",fontWeight:800,color:C.text,letterSpacing:"-0.03em"}}>Pick an event type</h3>
        <p style={{margin:"8px 0 0",fontSize:"0.82rem",color:C.text2,lineHeight:1.5}}>Or describe your event in the chat and I'll build the plan automatically.</p>
      </motion.div>
      ...
      <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:0.2}}
        ...
        {EVENT_TYPES_GRID.map((t,i)=>(
          <motion.button key={t.key}
            ...
            onMouseEnter={e=>{e.currentTarget.style.background=`${t.color}22`;e.currentTarget.style.borderColor=`${t.color}55`;e.currentTarget.style.boxShadow=`0 0 20px ${t.color}22`;}}
            onMouseLeave={e=>{e.currentTarget.style.background=C.surface;e.currentTarget.style.borderColor=C.border;e.currentTarget.style.boxShadow="none";}}>
            <span style={{fontSize:"1.5rem",lineHeight:1}}>{t.emoji}</span>
            <span style={{fontSize:"0.62rem",fontWeight:700,color:C.text2,letterSpacing:"-0.005em"}}>{t.label}</span>
          </motion.button>
        ))}
      </motion.div>
    </div>
...
export default function PlannerClient({ lang }) {
  ...
  const searchVendors = useCallback(async (service_type, search_term, filters={}) => {
    try {
      ...
      if (!data?.length) {
        ...
        if(r.ok){const j=await r.json();data=(j.data||[]).map(p=>({id:p.id,business_name:p.vendor_name||p.name,name:p.vendor_name||p.name,rating:p.rating,city:p.vendor_city||city||"",slug:p.vendor_slug||""}));}
      }
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      const res = await fetch("/api/planner/chat", {
        ...
        body: JSON.stringify({ messages:[...messages, userMsg], eventState }),
      });
  ...
  // Show welcome center content only when no conversation started yet
  ...
  return (
    <>
      ...
      <div style={{ height:`calc(100vh - ${SITE_HEADER_H}px)`, display:"flex", flexDirection:"column", background:C.bg, overflow:"hidden" }}>

        {/* ── Top bar ── */}
        <div style={{ height:PLANNER_TOP_H, flexShrink:0, background:"rgba(11,11,20,0.8)", backdropFilter:"blur(20px)", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 22px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <Link href={`/${lang}`} style={{ textDecoration:"none" }}>
              <motion.button whileHover={{ background:"rgba(255,255,255,0.08)" }} whileTap={{ scale:0.93 }}
                style={{ width:30, height:30, borderRadius:8, border:`1px solid ${C.border}`, background:"rgba(255,255,255,0.04)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"background 0.15s" }}>
                <ArrowLeft size={13} color={C.text2}/>
              </motion.button>
            </Link>
            <div style={{ width:1, height:18, background:C.border }}/>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              {/* Gradient pill label like Touri "AI Chat" */}
              <div style={{ display:"flex", alignItems:"center", gap:6, background:C.grad, borderRadius:100, padding:"5px 12px" }}>
          ...
          {hasEvent && (
            <motion.div initial={{opacity:0,x:10}} animate={{opacity:1,x:0}}
              style={{ display:"flex", alignItems:"center", gap:9, background:"rgba(255,255,255,0.05)", border:`1px solid ${C.border}`, backdropFilter:"blur(20px)", borderRadius:100, padding:"4px 12px 4px 9px" }}>
              ...
              <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:5, marginTop:2 }}>
                  <div style={{ width:48, height:2.5, background:"rgba(255,255,255,0.1)", borderRadius:10, overflow:"hidden" }}>
                    <motion.div
                      ...
                      style={{ height:"100%", background:C.grad, borderRadius:10 }}
        ...
        <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
          ...
          {/* ══ LEFT: Chat panel ══ */}
          <div style={{ width:440, flexShrink:0, display:"flex", flexDirection:"column", borderRight:`1px solid ${C.border}`, overflow:"hidden", position:"relative" }}>

            {/* Blob backgrounds — always visible */}
            <div style={{ position:"absolute", inset:0, pointerEvents:"none", zIndex:0 }}>
              <div style={{ position:"absolute", top:"-15%", right:"-15%", width:"70%", height:"65%", borderRadius:"50%", background:"radial-gradient(ellipse,rgba(249,115,22,0.22) 0%,transparent 65%)" }}/>
              <div style={{ position:"absolute", bottom:"5%", left:"-20%", width:"75%", height:"70%", borderRadius:"50%", background:"radial-gradient(ellipse,rgba(124,58,237,0.2) 0%,transparent 65%)" }}/>
              <div style={{ position:"absolute", top:"35%", left:"20%", width:"60%", height:"50%", borderRadius:"50%", background:"radial-gradient(ellipse,rgba(225,29,92,0.1) 0%,transparent 70%)" }}/>
            </div>

            {/* Messages / Welcome — above blobs */}
            <div style={{ flex:1, position:"relative", zIndex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
              <AnimatePresence mode="wait">
                {showWelcome ? (
                  <motion.div key="welcome" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0,scale:0.97}} transition={{duration:0.2}}
                    style={{ flex:1, overflow:"hidden" }}>
                    <ChatWelcome onPickType={(label) => sendMessage(`I want to plan a ${label}`)} />
                  </motion.div>
                ) : (
            ...
            <div style={{ flexShrink:0, padding:"12px 16px 18px", position:"relative", zIndex:2 }}>
              {/* Gradient border trick: outer gradient div + inner dark div */}
              <div style={{
                background: focused
                  ? "linear-gradient(135deg,#f97316,#e11d5c,#7c3aed)"
                  : `linear-gradient(135deg,rgba(255,255,255,0.12),rgba(255,255,255,0.06))`,
                borderRadius:16, padding:"1.5px",
                boxShadow: focused ? "0 0 30px rgba(249,115,22,0.18), 0 0 60px rgba(124,58,237,0.12)" : "none",
                transition:"box-shadow 0.25s",
              }}>
                <div style={{ background:"rgba(11,11,20,0.95)", borderRadius:"14.5px", padding:"10px 10px 10px 16px", display:"flex", alignItems:"center", gap:8, backdropFilter:"blur(20px)" }}>
                  <form onSubmit={e=>{e.preventDefault();sendMessage(input);}} style={{ display:"flex", flex:1, alignItems:"center", gap:8 }}>
                    ...
                    <motion.button type="submit" disabled={!canSend}
                      ...
                      style={{
                        ...
                        background: canSend ? "linear-gradient(135deg,#f97316,#e11d5c)" : "rgba(255,255,255,0.08)",
                        cursor: canSend?"pointer":"default",
                        display:"flex", alignItems:"center", justifyContent:"center",
                        boxShadow: canSend ? "0 4px 16px rgba(249,115,22,0.4)" : "none",
                        transition:"all 0.2s",
                      }}>
                      {loading
                        ? <motion.div animate={{rotate:360}} transition={{duration:0.9,repeat:Infinity,ease:"linear"}}><Loader2 size={14} color="rgba(255,255,255,0.5)"/></motion.div>
                        : <Send size={14} color={canSend?"#fff":"rgba(255,255,255,0.3)"} strokeWidth={2.2}/>
                      }
                    </motion.button>
                  </form>
                </div>
              </div>
              <p style={{ margin:"6px 0 0", fontSize:"0.62rem", color:C.text3, textAlign:"center" }}>
          ...
          {/* ══ RIGHT: Plan panel ══ */}
          <div style={{ flex:1, background:C.bgRight, overflowY:"auto", position:"relative" }}>
            <AnimatePresence mode="wait">
              {!hasEvent ? (
                ...
                <motion.div key="plan" initial={{opacity:0,x:16}} animate={{opacity:1,x:0}} transition={{duration:0.28,ease:[0.22,1,0.36,1]}}
                  style={{ padding:"22px 24px 40px" }}>
