...
const steps = [
  ...
  { label: "Processing", icon: Loader, desc: "Your order is being prepared" },
  { label: "Out for Delivery", icon: Truck, desc: "On the way to you" },
  { label: "Delivered", icon: CheckCircle, desc: "Order complete" },
];
      ...
      <div className="max-w-container mx-auto px-4 md:px-8">
        <ScrollReveal>
          <div className="max-w-xl mx-auto">
            <h1 className="text-3xl font-bold text-surface-900 mb-2">Track Your Order</h1>
            <p className="text-surface-400 mb-8">Enter your order details below to see the latest status.</p>
            ...
            <div className="mb-8">
              <h2 className="text-lg font-semibold text-surface-900 mb-6">
                {tracked ? "Your Order Status" : "Sample Order Status"}
              </h2>
              <div>
                {steps.map((step, i) => {
                    ...
                    <div key={i} className="flex items-start gap-4 mb-6 last:mb-0">
                      <div className="flex flex-col items-center">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                          isActive ? "bg-brand-600 text-white" : isDone ? "bg-brand-100 text-brand-600" : "bg-surface-100 text-surface-400"
                        }`}>
                          <Icon size={18} />
                        </div>
                        {i < steps.length - 1 && (
                          <div className={`w-0.5 h-8 mt-1 ${isDone ? "bg-brand-300" : "bg-surface-200"}`} />
                        )}
                      </div>
                      <div className="pt-1.5">
                        <p className={`text-sm font-semibold ${isActive ? "text-brand-600" : isDone ? "text-surface-700" : "text-surface-400"}`}>
                          {step.label}
                          {isActive && <span className="ml-2 text-xs bg-brand-50 text-brand-600 px-2 py-0.5 rounded-full font-medium">Current</span>}
                        </p>
            ...
            <div className="bg-brand-50 border border-brand-100 rounded-2xl px-5 py-4 text-sm text-surface-700">
              Need help?{" "}
              <a href="mailto:support@salooote.am" className="text-brand-600 font-medium hover:underline">
                Contact support at support@salooote.am
              </a>
            </div>
          </div>
        </ScrollReveal>
